...
public class DateUtil {
	...
	public static int getYearsWeeks(int year )  {
		int i = 52;
		if (DateUtil.pr == null) {
			try {
				DateUtil.pr = new Properties();
				InputStream st = DateUtil.class
						.getResourceAsStream("yearsweek.txt");
				DateUtil.pr.load(st);
				i = Integer.parseInt(DateUtil.pr.getProperty(String.valueOf(year)));
				st.close();
			} catch (Exception e) {
				DateUtil.pr = null;
				e.printStackTrace();
	...
	public static String addWeek(String week,int count){
		if (week != null && !week.equals("")){
			...
			if(weekInt>=count){
				return (weekInt-count+1)<10?(weekYear+"0"+(weekInt-count+1)):(weekYear+""+(weekInt-count+1));
			}else{
				int a=DateUtil.getYearsWeeks(weekYear-1);			
				return (weekYear-1)+""+(a-(count-weekInt)+1);
			}
		} else {
			return week;
		}
